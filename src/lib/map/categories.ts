...
export const MAP_CATEGORIES: CategoryMeta[] = [
  ...
  { id: "waterfalls", label: "Waterfalls & Lakes", iconName: "Droplets", color: "#06B6D4", bgClass: "bg-cyan-500/15", textClass: "text-cyan-300", borderClass: "border-cyan-500/40" },
...
export function getCategoryVisual(category?: string): CategoryMeta {
  ...
  if (cat === "WATERFALLS" || cat === "WATERFALL" || cat === "LAKE" || cat === "LAKES") return MAP_CATEGORIES.find((c) => c.id === "waterfalls") || MAP_CATEGORIES[5];
  if (cat === "NATURE") return MAP_CATEGORIES.find((c) => c.id === "nature") || MAP_CATEGORIES[6];
  if (cat === "BEACHES" || cat === "BEACH" || cat === "COAST") return MAP_CATEGORIES.find((c) => c.id === "beaches") || MAP_CATEGORIES[7];
  if (cat === "WILDLIFE") return MAP_CATEGORIES.find((c) => c.id === "wildlife") || MAP_CATEGORIES[8];
  if (cat === "PARKS" || cat === "PARK" || cat === "GARDEN" || cat === "GARDENS") return MAP_CATEGORIES.find((c) => c.id === "parks") || MAP_CATEGORIES[9];
  if (cat === "FAMILY" || cat === "THEME_PARK" || cat === "SCIENCE") return MAP_CATEGORIES.find((c) => c.id === "family") || MAP_CATEGORIES[10];
  if (cat === "ADVENTURE") return MAP_CATEGORIES.find((c) => c.id === "adventure") || MAP_CATEGORIES[11];
  if (cat === "CULTURE" || cat === "MUSEUM") return MAP_CATEGORIES.find((c) => c.id === "culture") || MAP_CATEGORIES[12];
  if (cat === "FOOD") return MAP_CATEGORIES.find((c) => c.id === "food") || MAP_CATEGORIES[13];
  if (cat === "SHOPPING") return MAP_CATEGORIES.find((c) => c.id === "shopping") || MAP_CATEGORIES[14];
