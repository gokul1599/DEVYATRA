...
async function generateAuditedExcel() {
  console.log("==================================================");
  console.log("🇮🇳 COMPILING 13-SHEET FORENSIC AUDITED WORKBOOK");
  console.log("==================================================");
  ...
  const auditResults = rawData.audit_results || [];
  const stateCoverageAudit = rawData.state_coverage_audit || [];

  console.log(`Loaded ${temples.length} temples and ${auditResults.length} audit results.`);

  const workbook = new ExcelJS.Workbook();
  workbook.creator = "India National Temple Master Database Project";
  workbook.lastModifiedBy = "National Temple Registry & Forensic Audit Authority";
  workbook.created = new Date();
  workbook.modified = new Date();

  function setupSheet(ws, title, columns, data, headerColor = 'FF1A237E') {
    ws.name = title;
  ...
  setupSheet(workbook.addWorksheet("MASTER_TEMPLES"), "MASTER_TEMPLES", MASTER_TEMPLES_COLUMNS, temples, 'FFE65100');

  // 2. AUDIT_RESULTS (Crimson Red Header for forensic review)
  setupSheet(
    workbook.addWorksheet("AUDIT_RESULTS"),
    "AUDIT_RESULTS",
    ["Temple_ID", "Temple_Name", "Field_Checked", "Existing_Value", "Source_Found", "Source_URL", "Source_Type", "Verified", "Problem", "Recommended_Action"],
    auditResults,
    'FFB71C1C'
  );

  // 3. STATE_COVERAGE_AUDIT (Teal / Green Header for official coverage audit)
  setupSheet(
    workbook.addWorksheet("STATE_COVERAGE_AUDIT"),
    "STATE_COVERAGE_AUDIT",
    ["State", "Official_Dataset_Found", "Official_Record_Count", "Records_Retrieved", "Existing_Records", "New_Records", "Duplicates", "Missing_Records", "Coverage", "Source", "Source_URL"],
    stateCoverageAudit,
    'FF004D40'
  );
  ...
  // 13. STATE_COVERAGE (Saffron Header)
  setupSheet(
  ...
  console.log("Writing 13-sheet workbook to disk...");
  await workbook.xlsx.writeFile(xlsxPath);
  console.log(`✓ Generated 13-sheet Audited Excel Workbook: ${xlsxPath}`);
}

generateAuditedExcel().catch(err => {
